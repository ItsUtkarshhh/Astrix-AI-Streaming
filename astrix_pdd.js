//---------------------------------------------------- Astrix — AI-Native Video Streaming Platform ------------------------------------------------------->
// Document Purpose
// This is the single source of truth for the design, architecture, and implementation plan of Astrix. It should be read before writing any code, updated as decisions evolve, and referenced throughout every phase of development.
...
// This project : Content creators and platform operators face difficulty in building a video streaming platform that handles the full lifecycle of video — from upload through processing, delivery, discovery, and personalisation — because building each piece in isolation is well-documented, but integrating them into a cohesive, scalable system requires deep knowledge of distributed systems, video protocols, AI tooling, and infrastructure that is rarely taught together.
//              : The secondary problem this project solves — for you as the builder : Existing portfolio projects for backend engineers demonstrate individual skills in isolation. There is no single project that credibly demonstrates system design depth, event-driven architecture, video processing pipelines, AI integration, observability, and production deployment simultaneously. Astrix is that project.

// What makes this non-trivial : The hard part is not building a video player. The hard part is designing a system where a 500MB video upload triggers an async pipeline that transcodes into multiple quality levels, enriches the video with AI-generated subtitles and tags, serves segments via a CDN with adaptive bitrate, and recommends the video to the right users — all without any single component blocking another, and all recoverable if any component fails.

// ------------------------------------------------------------ Goal & Vision ----------------------------------------------------------------------->
// General form : To enable [WHO] to [DO WHAT], using [HOW], so that [OUTCOME].
// This project : To build a production-grade, AI-native video streaming platform that enables content creators to upload and publish videos, and viewers to discover and stream personalised content — using an event-driven microservices architecture, HLS adaptive bitrate streaming, and an AI enrichment pipeline — so that the platform demonstrates real-world engineering depth across backend systems, distributed architecture, AI integration, and cloud infrastructure.

// What success looks like at completion : A user uploads a video → it is automatically transcoded into 360p, 720p, and 1080p HLS streams
...
// ------------------------------------------------------------ Users & Use Cases ----------------------------------------------------------------------->
// User Roles : Content Creator : Anyone who uploads videos to the platform - Upload, publish, and track performance of videos
...
// Use Cases by Role : Content Creator : Register and create an account
...
//                   : Viewer : Browse a personalised video feed on the homepage
...
//                   : Admin : View all uploaded content
//                           : Remove or flag content that failed moderation
//                           : View platform-wide analytics (uploads, streams, active users)
//                           : Manage user accounts

// ------------------------------------------------------------ Feature Breakdown — All Phases ----------------------------------------------------------------------->
// How to read this section : Features are grouped by phase. Each phase builds on the previous one. The question for every feature is : If I remove this, does the core flow break?* → Core (MVP)
//                                                                                                                                    : Does this make the product significantly better? → Phase 2
//                                                                                                                                    : Is this about scale, observability, or polish? → Phase 3+

// Phase 1 — MVP (Core Pipeline) : Goal : Upload a video, process it, stream it. Everything else is secondary.
//                               : Features : User Auth : Register, login, forgot password - OAuth & JWT
//                                          : Video Upload : Upload video file to S3/MinIO. Chunked upload for large files.
//                                          : Upload Progress : Real-time upload progress via WebSocket or SSE.
//                                          : Video Transcoding : FFmpeg transcodes uploaded video into 360p, 720p, 1080p HLS.
//                                          : HLS Delivery : Master `.m3u8` playlist + quality playlists + `.ts` segments served via CDN.
//                                          : Adaptive Bitrate Player : `hls.js`-powered player that switches quality based on bandwidth.
//                                          : Video Metadata : Title, description, tags, duration, resolution stored in PostgreSQL.
//                                          : Processing Status : Kafka event pipeline updates video status (uploading → processing → ready → failed).
//                                          : Basic Video Feed : Homepage grid of published videos, sorted by upload date.
//                                          : Basic Search : Keyword search on title and description via Meilisearch.
//                                          : Video Detail Page : Player + metadata + uploader info.

// Phase 2 — AI Enrichment Layer : Goal : Every video is enriched automatically the moment processing completes.
//                               : Features : Auto Subtitles : OpenAI Whisper transcribes audio → generates `.vtt` subtitle file.
//                                          : Multi-language Translation : Subtitles auto-translated to 5 languages (Hindi, Spanish, French, German, Japanese).
//                                          : Smart Thumbnails : CLIP model scores every 2-second keyframe → top 3 candidates presented to creator.
//                                          : Scene & Object Tagging : YOLOv8 detects objects in frames → auto-generates searchable tags.
//                                          : Content Moderation : NSFW and violence classifier runs on frames before video is made public.
//                                          : Semantic Search : Videos embedded via sentence-transformer → stored in pgvector → natural language queries work.
//                                          : Recommendation Engine : User vectors built from watch history → cosine similarity against video vectors → personalised feed.
//                                          : Watch History : Track per-user watch progress, resume position, and completed videos.
//                                          : Like & Save : Users can like and save videos to a personal library.

// Phase 3 — Scale, Observability & Infrastructure : Goal : Make it production-grade. Prove it scales. Monitor everything.
//                                                 : Features : Kubernetes Deployment - All services deployed as K8s pods with Helm charts.
//                                                            : Horizontal Pod Autoscaling : Transcoding worker pods scale based on Kafka queue depth.
//                                                            : Prometheus + Grafana : Metrics for every service — request rate, error rate, queue depth, transcoding time.
//                                                            : ClickHouse Analytics : View events streamed via Kafka → stored in ClickHouse → analytics dashboard (watch time, drop-off).
//                                                            : CDN Integration : CloudFront (prod) or MinIO (dev) for HLS segment caching at edge.
//                                                            : Load Testing : k6 scripts simulating 100 → 1000 concurrent streamers. Bottlenecks identified and fixed.
//                                                            : CI/CD Pipeline : GitHub Actions → Docker build → push to ECR → deploy to EKS.
//                                                            : Rate Limiting : Per-user and per-IP rate limits on all public APIs via Redis.
//                                                            : Structured Logging - Pino logger across all Node.js services → log aggregation via Loki.
//                                                            : Distributed Tracing - OpenTelemetry spans across services → trace a single video upload end-to-end.

// Phase 4 — Advanced Features & Polish : Goal : Feature completeness. Real product feel.
//                                      : Features : Notifications : In-app + email notifications (new upload from followed creator, processing complete).
//                                                 : Creator Analytics Dashboard : Per-video: views, watch time, audience drop-off curve, device breakdown.
//                                                 : Multiple Thumbnail Templates : Creators choose from 3 AI-generated thumbnail candidates or upload their own.
//                                                 : Video Chapters : Creators add chapter markers → shown in player timeline.
//                                                 : Shorts / Clips : Clip any 60-second segment of a video → shareable as a short.
//                                                 : Comments & Replies : Threaded comments on videos.
//                                                 : Subscriptions : Users follow creators → personalised feed prioritises followed creators.
//                                                 : Mobile Responsive UI : Full mobile optimisation across all pages.
//                                                 : Demo Video + Architecture Docs - 15-min recorded walkthrough of the platform under load. Public architecture doc.

// ------------------------------------------------------------ Tech Stack & Decisions ----------------------------------------------------------------------->
// Every decision here has a reason. The reason is more important than the decision.

// Frontend : React + Vite or NextJs - React for component reusability and ecosystem. Vite over CRA — faster dev server, better DX. Next.js skipped because SSR adds complexity without meaningful SEO benefit for a streaming platform (content is behind auth anyway).
//          : Tailwind CSS or Bootstrap - Utility-first means no context-switching between files. Faster to iterate on UI without a design system.
//          : hls.js or Video.js - Lightweight, focused solely on HLS playback. Video.js is heavier with plugins. hls.js gives direct control over quality switching logic.
//          : TanStack Query - Handles caching, background refetch, loading and error states cleanly. Eliminates the useEffect-for-data-fetching anti-pattern.
//          : Zustand, Redux or Context API - Lightweight global state for auth and player state. Redux is overkill here. Context causes unnecessary re-renders at scale.

// Backend : NodeJs + Fastify, Express, Spring Boot or Django - Node's event-driven I/O handles thousands of concurrent HTTP range requests (HLS segment serving) efficiently. Fastify over Express — schema-based validation, better TypeScript support, measurably faster. Spring Boot is a stronger choice for a Java team but adds JVM overhead and slower startup time that hurts Kubernetes pod scaling
...
//         : TypeScript throughout - Type safety catches interface mismatches between services early. In a multi-service system, the cost of a wrong payload shape is high. TS pays for itself within the first week.

// Message Queue : Apache Kafka,  RabbitMQ, BullMQ or AWS SQS - Kafka is a distributed log, not just a queue. Events are retained and replayable — if a transcoding worker crashes, the event is not lost, the consumer group resumes from last committed offset. RabbitMQ is a better choice for simple job queues but doesn't give you event replay or the consumer group model. BullMQ (Redis-backed) is used for simpler background jobs within a service.

// Databases : PostgreSQL - Users, videos, metadata, subtitles, watch history, likes | Relational integrity matters here — a video belongs to a user, a subtitle belongs to a video. ACID transactions for writes that must not partially succeed. pgvector extension adds vector similarity search without a separate service.
...
//           : Meilisearch (Phase 1) → Elasticsearch (Phase 3) - eilisearch is lightweight (~100MB RAM) and trivial to set up — perfect for development and early production. Elasticsearch is more powerful (custom analysers, complex queries, horizontal scaling) but costs 1–1.5GB RAM minimum. Swap at Phase 3 when load testing justifies it.

// Storage & CDN : MinIO (local dev) - S3-compatible object storage that runs locally in Docker. Identical API to AWS S3, so switching to real S3 in production requires zero code changes.
...
// Infrastructure : Docker + Docker Compose - Every service containerised. Local dev runs the full stack with one command
...
//                    : Services communicate primarily via Kafka events, not direct HTTP calls. This means - Services are independently deployable
